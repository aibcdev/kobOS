...
export function AlvenRoleStage({
  ...
  if (mode === "video" && videoSrc) {
    return (
      <div
        className={cn(
          "overflow-hidden rounded-[1.75rem] bg-[#ecece9]",
          className,
        )}
      >
        <video
  ...
  return (
    <div
      className={cn(
        "relative overflow-hidden rounded-[1.75rem] bg-[#ecece9] p-5 sm:p-8",
        className,
      )}
    >
      <span
        className="absolute right-5 top-5 text-[0.65rem] font-semibold tracking-wide"
        style={{ color: accent }}
      >
        LANGOSTERIA*
      </span>
      <div className="relative mx-auto flex max-w-md flex-col gap-3 pt-4">
        <span className="absolute bottom-4 left-[1.15rem] top-8 w-px bg-line-strong" />
