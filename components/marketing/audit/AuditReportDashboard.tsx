...
type NavId = "overview" | "seo" | "reviews" | "conversion" | "competitors";
...
export function AuditReportDashboard({
  ...
  const { data } = useAuditBenchmarkPoll(audit.id, benchmarkInitial, { enabled: unlocked });
  ...
  const healthLabel = onlineHealthLabel(overall);
  ...
  const competitorsFromPlaces = payload.competitors.some((c) => c.source === "places");
        ...
        <aside className="hidden w-56 shrink-0 border-r border-[var(--color-hairline)] bg-[var(--color-surface-cream)]/80 px-4 py-8 lg:block">
          ...
          <div className="mt-10 rounded-[var(--radius-md)] bg-[var(--color-ink)] p-4 text-[var(--color-text-inverse)]">
            ...
            <p className="mt-2 text-sm leading-snug">
              You&apos;re losing orders when guests hit broken links or weak menu photos—fix those first.
            </p>
            {unlocked ? (
              <Link
                href="/dashboard"
            ...
            <div className={!unlocked ? auditBlurGate : ""}>
              <div className="mb-8 flex flex-col items-center gap-6 text-center md:flex-row md:items-end md:justify-between md:text-left">
                <div className="md:text-left">
                  ...
                  >
                    {unlocked ? "Your performance audit" : "Your AI visibility report"}
                  </h1>
                  <p className="type-body-sm mt-2 text-[var(--color-muted)]">
                    {unlocked
                      ? `Generated ${new Date(audit.updatedAt).toLocaleDateString("en-GB", { dateStyle: "medium" })} · 4 platforms analyzed`
                ...
                <div className="flex flex-col items-center gap-3 rounded-[var(--radius-md)] border border-[var(--color-hairline)] bg-white p-6 shadow-sm sm:flex-row sm:gap-6">
                  <ScoreRing score={overall} size={unlocked ? 120 : 140} />
                  <div className="text-center sm:text-left">
                    <p className="type-caption font-medium uppercase tracking-wide text-[var(--color-muted-medium)]">
                      {marketingCopy.graderReport.onlineHealth}
                    </p>
                    <p className={`type-title-md mt-1 font-semibold ${tone.text}`}>{healthLabel}</p>
                    {unlocked ? (
                      <p className="type-body-sm mt-2 max-w-[200px] text-[var(--color-muted)]">
              ...
              {(activeNav === "overview" || activeNav === "seo") && (
                <div className="mb-8 grid gap-4 sm:grid-cols-3">
                  <PillarCard label="SEO score" value={data.seoScore} max={100} />
                  <PillarCard label="Reviews health" value={reviewsScore} max={100} />
                  <PillarCard label="Conversion" value={data.conversionScore} max={100} urgent={data.conversionScore < 30} />
                </div>
              )}

              {(["overview", "seo", "reviews", "competitors"] as NavId[]).includes(activeNav) && (
                <div
                  className={`mb-8 grid gap-6 ${
                    activeNav === "overview" ? "lg:grid-cols-[1.4fr_1fr]" : "max-w-2xl"
                  }`}
                >
                  {(activeNav === "overview" || activeNav === "seo" || activeNav === "reviews") && (
                  <section className="rounded-2xl border border-[var(--color-hairline)] bg-white p-6 shadow-sm">
                    <div className="flex items-center justify-between gap-4">
                      <h2 className="font-head text-lg font-semibold">Top recommendations</h2>
                      <span className="rounded-full bg-red-50 px-2.5 py-1 text-[10px] font-bold uppercase text-red-600">
                        {Math.min(payload.issues.length, 3)} critical
                      </span>
                    </div>
                    <ul className="mt-6 space-y-5">
                      {payload.issues.slice(0, 3).map((issue, i) => (
                        <li key={issue.title} className="flex gap-4 border-b border-[var(--color-hairline)] pb-5 last:border-0 last:pb-0">
                  ...
                  {(activeNav === "overview" || activeNav === "competitors") && (
                    <section className="rounded-2xl border border-[var(--color-hairline)] bg-white p-6 shadow-sm">
                      ...
                      {!competitorsFromPlaces ? (
                        <p className="mt-1 text-xs text-[var(--color-muted-medium)]">
                          Estimated local peers — location not confirmed yet.
                        </p>
                      ) : (
                        <p className="mt-1 text-xs text-[var(--color-muted-medium)]">Nearby restaurants from Google Places.</p>
                      )}
                      ...
                      <div className="mt-6 rounded-xl bg-[var(--color-surface-cream)] p-4 text-sm leading-relaxed text-[var(--color-muted)]">
                        <span className="font-semibold text-[var(--color-accent)]">AI insight · </span>
                        Competitors with stronger local SEO pages are capturing high-intent searches in {audit.city}.
                      </div>
                    </section>
                  )}
                </div>
              ...
              {unlocked && data.benchmarkV1 ? (
                <section id="seo" className="mb-8 rounded-2xl border border-[var(--color-hairline)] bg-white p-6 shadow-sm">
              ...
              {showVideo && data.benchmarkV1Media?.videoPresentationQuality ? (
                <section className="mb-8 space-y-4 rounded-2xl border border-[var(--color-hairline)] bg-white p-6 shadow-sm">
