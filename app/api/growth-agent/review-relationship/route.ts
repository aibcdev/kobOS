...
export async function POST(req: Request) {
  ...
  const result = await generateReviewRelationship(parsed.data.restaurantId, parsed.data.reviewId);
  if (!result.ok) {
  ...
  try {
    await persistGrowthGeneration({
      ...
      prompt: `review-relationship restaurant=${parsed.data.restaurantId} review=${parsed.data.reviewId}`,
      payload: result.data,
    });
  } catch (e) {
    console.error("persistGrowthGeneration review-relationship", e);
  }
