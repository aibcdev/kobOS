...
import { pauseLiveOffer } from "@/lib/demand-engine/actions";
...
export async function POST(req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  const result = await pauseLiveOffer(parsed.data.restaurantId, id);
  if (!result.ok) {
    return NextResponse.json({ error: result.error }, { status: 404 });
  }

  return NextResponse.json({ ok: true, liveOffer: result.liveOffer });
}
