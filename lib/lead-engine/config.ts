...
export function getLeadEngineConfig(): LeadEngineConfig {
  ...
  return {
    ...
    instagramMax: Math.max(1000, Number(process.env.LEAD_ENGINE_INSTAGRAM_MAX?.trim() || "10000") || 10000),
    locationMax: Math.max(1, Number(process.env.LEAD_ENGINE_LOCATION_MAX?.trim() || "3") || 3),
    platformTopPct: Math.min(100, Math.max(5, Number(process.env.LEAD_ENGINE_PLATFORM_TOP_PCT?.trim() || "30") || 30)),
    requireStaleWebsite: process.env.LEAD_ENGINE_REQUIRE_STALE_WEBSITE?.trim() !== "0",
